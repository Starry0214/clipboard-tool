...
- Android 10+ 后台读剪贴板受限，实时监听需无障碍服务（用户接受，但**不要常驻通知**）。
...
- **Android 端新建**：Kotlin + Jetpack Compose，无障碍服务监听剪贴板（无常驻通知），App 内历史列表。
...
- **手机 → 电脑**：手机复制 → 无障碍监听回调 → 本地入库 → 上传（文本直发 / 图片文件先 POST /media）→ 服务端推送 → 电脑 SyncService 收到 → **写入历史库（source=phone）**，用户在 Win+V 悬浮列表中选择粘贴。电脑端不自动写系统剪贴板。
...
- **模块**：
  - `ClipboardListener`（AccessibilityService）：服务启用时注册 `OnPrimaryClipChangedListener`，回调中读 ClipData（无障碍服务豁免 Android 10+ 后台剪贴板读限制），解析文本/图片/文件（content Uri 经 ContentResolver 读流），转交 SyncClient；**不挂前台服务、无常驻通知**。
  - `SyncClient`：OkHttp WebSocket + HTTP；心跳、重连、双镜像回退、上传/下载。
  - `LocalStore`：Room 历史库（本地全量），字段含来源（local/pc）、类型、时间、内容引用。
  - `MainActivity`：历史列表（时间分组、来源标签、类型筛选）、点击条目写回剪贴板；登录页（注册/登录）；设置页（账号/设备管理、无障碍开启引导、自启动引导防 MIUI 清理）。
- **MIUI 保活**：首启引导用户在"自启动管理"允许后台运行 + 电池优化白名单；App 打开时检测无障碍服务是否存活，未存活则提示。
...
| M3 | Android 端：脚手架、无障碍监听、SyncClient、Room、历史 UI、登录/设备管理、保活引导 | 真机跑通登录 + 复制监听 + 历史 + 回填 |
...
- 纯无障碍服务在 MIUI 上可能被后台清理导致漏记——通过自启动引导缓解，不保证 100% 存活。
