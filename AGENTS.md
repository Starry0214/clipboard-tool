...
- 版本号由两个 `csproj` 的 `<Version>` 控制（当前 1.4.11，**两处必须一致**），自动更新与它比较。`Launcher/embedded/ClipboardToolApp.exe`（被 gitignore）是主程序 exe 副本，解压时以实际 FileVersion 为准（`GetExeVersion` 自愈，见发布节）。
...
- **WPS 复制文件不展示（2026-08-20 修复，提交 9f47771）**：两个叠加根因——① ClipboardMonitor.Capture() 捕获优先级原为 文本→图片→文件，WPS 复制文件时剪贴板同时含文件名文本与 FileDrop，文本分支先命中直接 return，文件名被存成文本条目、文件永不入库；改为 图片→文件→文本。② 文件读取用 File.ReadAllBytes/File.OpenRead（默认 FileShare.Read），而 WPS 以 FileAccess.ReadWrite + FileShare.Read（独占写）打开 docx，共享声明冲突 → 读失败 → hash null → Add 静默跳过（旧版"捕获文件"日志在成功判断外无条件打印，极具误导）。全项目文件字节读取统一改 FileShare.ReadWrite（ClipboardMonitor/ClipboardStore/SyncService 新增 ReadAllBytes 辅助）；文件不可读时自动重试 5 次（防占用/OneDrive 云占位）并明确打日志。排查手法：对比日志"捕获文件"与 DB 实际条目，验证 WPS 进程占用时 File.OpenRead 失败而 FileShare.ReadWrite 成功。
- **手机图片按文件同步被分类成文件（2026-08-21 修复）**：手机端分享/保存相册后再复制图片时，Android 常以 clip_file 携带 .jpg/.png 文件名发送（非剪贴板位图）；Windows SyncService.ApplyRemote 收到 clip_file 后盲目按 type=file 入库到 files/、无缩略图、显示在"文件"分类。修复：ApplyRemote 对 clip_file 用 IsImageName(m.Name)（按扩展名 .jpg/.jpeg/.png/.gif/.bmp/.webp/.tiff）判断，属图片扩展名则按 image 入库到 images/ 并生成缩略图。排查手法：sqlite3 clipboard.db "SELECT id,type,source,content FROM entries WHERE type='file' AND content LIKE '%Image_%'" 找错分条目；服务器 messages 表 payload 里 name=图片扩展名但 type=clip_file 即为该场景。
